...
class Api extends Controller
{
    ...
    public function send2Customer()
    {
        if (request()->isPost()) {
            ...
            $this->curlPost(config('xiaotian_socketio.api_url'), $param);
    ...
    public function doRelink()
    {
        if (request()->isPost()) {
            ...
            $port = config('xiaotian_socketio.http_port');
            $relinkInfo = $this->curlPost('http://127.0.0.1:' . $port, $param);
            $relinkInfo = json_decode($relinkInfo, true);

            // 通知转接车成功
            $this->curlPost(config('xiaotian_socketio.api_url'), $relinkInfo['data']);
    ...
    public function closeUser()
    {
        if (request()->isPost()) {
            ...
            // 通知关闭消息
            $this->curlPost(config('xiaotian_socketio.api_url'), $returnInfo['data']);
