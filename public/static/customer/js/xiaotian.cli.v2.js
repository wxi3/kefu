...
function callBackOnMessage (data) {
    ...
    switch(data['cmd']) {
        ...
        // 常见问题
        case 'comQuestion':
            comQuestion = xiaotian.showMessage(data.data);
            if (0 == isOpen) {
        ...
        case 'answerComQuestion':
            $(".chat-box").append(xiaotian.showMessage(data.data));
            wordBottom();
            break;

        // 问候语
        case 'hello':
            helloWord = xiaotian.showMessage(data.data);
            break;
...
function userInitHandle(data) {
    ...
    if (400 == data.code) {
        ...
        clearInterval(reConnectInterval);
        $(".chat-box").append(xiaotian.showSystem(data.msg));
        isConnect = 0;
        $.hideLoading();
...
function chatMessageHandle(data) {
    ...
    var chatMsg = xiaotian.showMessage(data.data);
...
function afterSendHandle(data) {
    ...
    if (400 == data.code) {
        ...
        var msg = xiaotian.send(input, customer.avatar, data.data);
    } else if (0 == data.code) {
        var msg = xiaotian.send(input, customer.avatar, data.data);
    }
...
function relinkHandle(data) {
    ...
    $(".chat-box").append(xiaotian.showSystem(data.data.msg));
    wordBottom();
}
    ...
    if (2 == type) {
        ...
        window.addEventListener('message', function(event){
            ...
            if('open_chat' == msg.cmd) {
                ...
                localStorage.setItem('xiaotian_referrer', msg.data.referrer);
...
function robotService () {
    ...
    if (2 == type) {
        ...
        var robotHello = xiaotian.showMessage({
            read_flag: 2,
            chat_log_id: 0,
            content: robot_hello,
            time: xiaotian.getCurrDate(),
...
function robotAnswer () {
    ...
    var msg = xiaotian.completeReadSend(input, customer.avatar, 0);
    $(".chat-box").append(msg);
    $("#textarea").val('');
    wordBottom();
...
$(function () {
    ...
    $("#face").on('click', function (e) {
        ...
        layui.use(['layer'], function () {
            ...
            var isShow = $(".layui-xiaotian-face").css('display');
            if ('block' == isShow) {
                layer.close(index);
                return;
            }
...
function sendMessage(inMsg) {
    ...
    if (input.substr(0, 4) != 'img[' && input.substr(0, 5) != 'file(') {
        input = xiaotian.autoReplaceUrl(input);
    }
        ...
        if(0 == res.code && res.data.length > 0) {
            ...
            $.each(res.data, function (k, v) {
                if(v.type == 'mine') {
                    ...
                    _html += xiaotian.showMyChatLog(v);
                } else if(v.type == 'user'){

                    _html += xiaotian.showMessage({time: v.create_time, avatar: v.from_avatar, content: v.content, chat_log_id: v.log_id, read_flag: v.read_flag});
                }
            });
...
function autoAnswer (obj) {
    ...
    var msg = xiaotian.completeReadSend(question, customer.avatar, 0);
    $(".chat-box").append(msg);
...
function robotAutoAnswer (obj) {
    ...
    $.post('/index/robot/autoAnswer', {
        ...
        var msg = xiaotian.completeReadSend(question, customer.avatar, 1);
        $(".chat-box").append(msg);

        var msg = xiaotian.showMessage({
            read_flag: 2,
            chat_log_id: 0,
            content: res.msg,
            time: xiaotian.getCurrDate(),
        ...
        layer.open({
            ...
            yes: function(index, layero){
                ...
                $.post('/index/index/praise', {
                    ...
                    $(".chat-box").append(xiaotian.showSystem(res.msg));
                    isPraise = 1;
                    socket.close();
                    wordBottom();
                    kefuCode = 0;
    ...
    if (noAnswerCloseTime > 0){
        ...
        window.msgSpaceInterval = setInterval(function () {
            ...
            if (window.msgSpaceSeconds >= noAnswerCloseTime){
                ...
                isPraise = 1;
                socket.close();
                $(".chat-box").append(xiaotian.showSystem('由于您长时间未进行对话，本次对话已结束'));
                wordBottom();
            }
        },1000);
    }
