...
;!function (win, doc) {
    ...
    var xiaotian = function () {
        ...
        , replaceContent = function(content) {
            ...
            content = (content || '').replace(/&(?!#?[a-zA-Z0-9]+;)/g, '&amp;')
                ...
                .replace(/img\[([^\s]+?)\]/g, function (img) {  // 转义图片
                    return '<img class="layui-xiaotian-photos" src="' + img.replace(/(^img\[)|(\]$)/g, '') + '" style="max-width: 100%;width: 100%;height: 150px">';
                })
                .replace(/file\([\s\S]+?\)\[[\s\S]*?\]/g, function (str) { // 转义文件
                    var href = (str.match(/file\(([\s\S]+?)\)\[/) || [])[1];
                    var text = (str.match(/\)\[([\s\S]*?)\]/) || [])[1];
                    if (!href) return str;
                    return '<a class="layui-xiaotian-file" href="' + href + '" download target="_blank"><i class="layui-icon">&#xe61e;</i><cite>' + (text || href) + '</cite></a>';
                })
        ...
        , replaceEdit = function(content) {
            ...
            return content.replace(/style\s*=\s*('[^']*'|"[^"]*")/g, function(style) {
                ...
                .replace(/src\s*=\s*('[^']*'|"[^"]*")/g, function(src) {
                    return src.replace(/\s+/g,"").replace(/\"/g, "")+ ' class=layui-xiaotian-photos style=max-width:100%;width:100%;height:150px';
                })
        ...
        , showFaces = function () {
            ...
            var _html = '<div class="layui-xiaotian-face"><ul class="layui-clear xiaotian-face-list">';
            $.each(alt, function (index, item) {
                _html += '<li title="' + item + '" onclick="xiaotian.checkFace(this)"><img src="/static/common/images/face/' + index + '.gif" /></li>';
            });
            _html += '</ul></div>';
    ...
    xiaotian.prototype.init = function (conf) {

    };

    // 发送消息
    xiaotian.prototype.send = function (content, avatar, flag) {
    ...
    // 显示消息
    xiaotian.prototype.showMessage = function (data) {
    ...
    // 发送已读dom
    xiaotian.prototype.completeReadSend = function (content, avatar, flag) {
    ...
    // 显示聊天信息中，我发送的消息
    xiaotian.prototype.showMyChatLog = function (data) {
    ...
    // 显示系统消息
    xiaotian.prototype.showSystem = function (msg) {
        return [
            '<div class="clearfloat ">'
            ,'<div class="author-name">'
    ...
    // 获取当前时间
    xiaotian.prototype.getCurrDate = function () {
        return new Date().format("yyyy-MM-dd hh:mm:ss");
    };

    // 展示表情
    xiaotian.prototype.showFaces = function () {
        return showFaces();
    };

    // 选择表情
    xiaotian.prototype.checkFace = function (obj) {
        var word = $("#textarea").val() + ' face' + $(obj).attr('title') + ' ';
        $("#textarea").val(word).focus();

        $(".layui-xiaotian-face").hide();
    ...
    // 展示大图
    xiaotian.prototype.showBigPic = function () {
        $(".layui-xiaotian-photos").on('click', function () {
            var src = this.src;
            layer.photos({
                photos: {
                    data: [{
    ...
    // 消息声音提醒
    xiaotian.prototype.voice = function () {
        $("#xiaotian-index-audio").get(0).play();
    };

    // 内容替换
    xiaotian.prototype.replaceContent = function (text) {
        return replaceContent(text);
    };

    // 格式化标签
    xiaotian.prototype.replaceEdit = function(text) {
        return replaceEdit(text);
    };

    // 自动识别url连接
    xiaotian.prototype.autoReplaceUrl = function(text) {
    ...
    console.log('本系统由xiaotian客服提供服务，官网地址: https://xiaotian.xmf3.cn');
    win.xiaotian = new xiaotian();
}(window, document);