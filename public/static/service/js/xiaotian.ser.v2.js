    ...
    switch(data['cmd']) {
        ...
        case 'afterSend':
            ...
            if (400 == data.data.code) {
                ...
                var msg = xiaotian.send(input, kefuUser.avatar, data.data.data);
        ...
        case 'praiseKfCB':
            if (data.data.code == 0) {
                $(".chat-box").append(xiaotian.showSystem("已发送评价"));
                wordBottom(activeUser);
            } else {
                layer.msg(data.data.msg);
            }
...
$(function () {
    ...
    $(".word").hover(function () {
        ...
        $("#content").html(xiaotian.replaceContent(xiaotian.replaceEdit(($(this).attr("data-content")))));
    }, function () {
        $("#word-box").hide();
    });
    ...
    $.each(preWord, function (k, v) {
        comWord.push({
            value: xiaotian.replaceEdit(v.value),
            data: v.data
        })
    });
...
function sendWord(obj, flag) {
    ...
    if (flag === 1) {
        sendMessage(xiaotian.replaceEdit($(obj).attr('data-content')));
    } else if (flag === 2) {
        $("#textarea").val(xiaotian.replaceEdit($(obj).attr('data-content')));
    }
...
function showMessage(data) {
    ...
    // 展示聊天信息
    var chatMsg = xiaotian.showMessage(data);
...
function sendMessage(inMsg) {
    ...
    if ('ws' == protocol) {
        ...
        $.post('/index/api/send2Customer', {
            ...
            var msg = xiaotian.send(input, kefuUser.avatar, res.data);
            if (0 == nowModel) {
                $("#ct-" + activeUser).append(msg);
