...
class Index extends Controller
{
    ...
    public function chatBoxJs()
    {
        ...
        if (1 == config('xiaotian_socketio.default_box_link_flag')) {
        ...
        $time = time();
        $this->assign([
            'domain' => config('xiaotian_socketio.domain'),
            'seller' => $sellerCode,
            'time' => $time,
            'token' => md5($sellerCode . config('xiaotian.salt') . $time),
            'style' => $myStyle,
            'baseCss' => $baseCss
        ]);
    ...
    public function cliBox()
    {
        ...
        $safeToken = md5($sellerCode . config('xiaotian.salt') . $time);
        if($token != $safeToken) {
            return $this->fetch('error');
        }
        ...
        $nowTime = time();
        $safeToken = md5($sellerCode . config('xiaotian.salt') . $nowTime);
        ...
        $this->assign([
            'port' => config('xiaotian_socketio.socket_port'),
            'nowTime' => $nowTime,
            'token' => $safeToken,
            'seller' => $sellerCode,
            ...
            'direct_kefu' => '',
            'type' => 1, // 弹层显示
            'socket' => config('xiaotian.protocol') . config('xiaotian.socket') . '/' . $token,
            ...
            'referer' => isset(request()->header()['referer']) ? request()->header()['referer'] : config('xiaotian.domain'),
    ...
    public function chat()
    {
        ...
        // 校验接入域名
        if (1 == config('xiaotian_socketio.default_link_flag') && (!isset($_SERVER['HTTP_REFERER']) ||
        ...
        $nowTime = time();
        $safeToken = md5($sellerCode . config('xiaotian.salt') . $nowTime);
        $token =  $sellerCode . '-' . $nowTime  . '-' . md5($sellerCode . config('xiaotian.salt') . $nowTime);
