...
function callBackOnMessage(res) {
    ...
    switch(data['cmd']) {
        ...
        case 'afterSend':
            ...
            var content = xiaotian.replaceContent(data.data.msg);
            ...
            _html += '<p class="read-flag no-read" data-msg-id="' + data.data.data + '">未读</p></li>';
        ...
        // 动态删除访客列表
        case 'removeQueue':
            $("#h-" + data.data.customer_id).parent().remove();
            break;
...
function showMessage(info) {
    ...
    var content = xiaotian.replaceContent(info.content);
        ...
        if(0 == res.code && res.data.length > 0){
            ...
            for(var i = 0; i < len; i++){
                ...
                if('mine' == item.type){
                    ...
                    _html += '<div class="layim-chat-text">' + xiaotian.replaceContent(item.content) + ' </div>' + ifRead + '</li>';

                }else {

                    var readFlag = '<div class="layim-chat-text check-read" data-msg-id="' + item.log_id + '">' + xiaotian.replaceContent(item.content) + '</div>';
                    if (2 == item.read_flag) {
                        readFlag = '<div class="layim-chat-text check-read" data-msg-id="' + item.log_id + '">' + xiaotian.replaceContent(item.content) + '</div>';
                    }
